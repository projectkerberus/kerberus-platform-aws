locals {
  crossplane_providers = merge({}, { "aws-provider" : module.configure_crossplane.provider })
  crossplane_secrets   = merge({}, { "aws-creds" : module.configure_crossplane.secret })
}

module "configure_crossplane" {
  source  = "projectkerberus/crossplane/kerberus//modules/aws-crossplane"
  version = "0.2.1"
...
module "kerberus_dashboard" {
  ...
  github_client_id      = var.github_app_id
